...
| ❸ | **力矩臂用錯** | 彈簧為水平方向，其對 B 的力臂是 A 點的**鉛垂高度** $L\cos\theta$，不是桿長 $L$；重力為鉛垂方向，其力臂是 A 點的**水平偏距** $L\sin\theta$ | 小角度下 $L\cos\theta\to L$、$L\sin\theta\to L\theta$，但推導時應先寫精確形式再線性化，才不會把兩個力臂互換 |
...
![SD-2010-1 自由體圖：倒擺 AB，彈簧水平力 Fs = kLsinθ 的力臂為 Lcosθ、重力 mg 的力臂為 Lsinθ](figs/SD-2010-1-fig-1-fbd.svg)

*圖 1　自由體圖。彈簧是**水平力**，力臂取 A 點的鉛垂高度 $L\cos\theta$；重力是**鉛垂力**，力臂取 A 點的水平偏距 $L\sin\theta$。兩者互換是本題最常見的失分——互換後恢復項與傾覆項會剛好對調。*
...
![SD-2010-1 位能曲線：r = kL/mg 取 1.60 / 1.00 / 0.55 三種情形，θ=0 分別為谷、平、峰](figs/SD-2010-1-fig-2-stability.svg)
...
除了 $\theta=0$ 外，當 $kL > mg$ 時還存在第二組平衡位置 $\cos\theta^\* = \dfrac{mg}{kL}$。
但因 $kL>mg$ 時 $\cos\theta^\*<1$ 有解、且該處 $\partial^2V/\partial\theta^2<0$，故 $\theta^\*$ 為**不穩定**平衡，$\theta=0$ 才是穩定平衡——與（一）的結論一致。
當 $k \to mg/L$ 時 $\theta^\* \to 0$，兩個平衡點合併，即分岔（bifurcation）發生的臨界狀態。
...
> 主體計算經重新驗算正確，未更動：
> $V=mgL\cos\theta+\frac12k(L\sin\theta)^2$，$\left.\partial^2V/\partial\theta^2\right|_0=kL^2-mgL>0\Rightarrow k>mg/L$；
> $mL^2\ddot\theta+(kL^2-mgL)\theta=0$，$\omega=\sqrt{k/m-g/L}$。力矩法（§5.1）獨立驗算結果相同。
