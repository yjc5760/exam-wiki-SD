...
- 2026-09-02｜FIX + STRUCT-DIAGRAM｜**使用者指定複查並修正 SD-2025-1、SD-2007-1、SD-2020-4、SD-2018-3、SD-2012-3 五題，並補向量圖解。**

  ⚠️ **規則 1 例外聲明**：本次修改了 `raw/solutions/SD-YYYY-N/*.md`（規則 1 保護範圍）。依據是使用者明確指示「幫我檢查並修正下列題目解題」。所有更正皆經 ① Python 重算或特徵值實算、② 與考卷原文（`raw/exams/`）比對、③ 與《建築物耐震設計規範及解說》條文交叉查證，三者齊備才動手。五題 verificationStatus 一律維持 `unverified`，待人工複驗。

  **規範面的共同勘誤（三處，影響多題）：**
  ① **中小度地震回歸期**：原寫「回歸期 72 年、50 年超越機率 50%」——那是美國 ASCE 41 的 *frequent* 水準。我國規範解說 1.2 明定為**回歸期約 30 年、50 年超越機率約 80%**（驗算：1−(1−1/30)^50 = 81.6% ✓）。已修正 SD-2020-4。⚠️ **SD-2024-3 有同一錯誤，未在本次委託範圍，僅登錄待辦。**
  ② **α_y 的定義**：原寫「超強係數，α_y ≤ 1」「降伏地震係數 = V_y/W」——皆錯。規範解說 2.9：α_y 為**起始降伏地震力放大倍數**，無因次且 **≥ 1**（RC 極限強度設計 1.5、鋼構極限設計 1.0、鋼構容許應力設計 1.2）。SD-2025-1 題目自己給 α_y = 1.4 即為反證。已修正 SD-2007-1、SD-2020-4。
  ③ **R_a 的 1.5 由來**：原寫「考慮施工品質變異與韌性不確定性的保守係數／參考 NEHRP」——規範解說 2.9 的理由是**設計地震下只容許非彈性位移達極限值之 2/3**，因最大考量地震約為設計地震的 1.5 倍（S_DS = ⅔S_MS）。是性能設計的分層邏輯，不是統計安全係數。已修正 SD-2007-1、SD-2020-4。

  **逐題更正：**
  - **SD-2025-1**（數值＋論述）：ΣF_i h_i 由 573.3 更正為 **573.1**、M₀ 由 621.3 更正為 **621.1** kN·m（原文對 0.011468 過早四捨五入）；F₂ 1.606→1.605、F₆ 4.817→4.816。§5.2 原稱本式為「舊版規範格式、新版改以 S_DS/S_D1 表示」——**錯誤**，此式即現行規範 2.2 之最小設計水平總橫力式，S_DS/S_D1 是建構 S_aD 譜形的參數而非取代它；改寫並補上規範原式的 (S_aD/F_u)_m 修正與 F_u 的 R→R_a→F_u 換算。§5.3 原把 h_eq = 0.725H 高於 2H/3 全歸因於 F_t——實際上 8 層離散樓層本身已給出 Σwh²/Σwh = 0.708H，F_t 只再加 0.017H，已改為三段對照。
  - **SD-2007-1**（公式＋觀念）：題目重述漏抄**用途係數 I**（考卷原文為 V = I·S_aD/(1.4α_yF_u)·W），且把 S_aD 誤寫為 S_aD1，已更正。刪去錯誤的「F_u = V_e/V_y = S_aD1/α_y」，改以規範的正確力量鏈重推：V → P_y = α_yV → P_u = 1.4α_yV → F_u = V_e/P_u，並補上規範解說 2.2「外力須調升至 1.4P_y 結構才達最大側力 P_u」作為 1.4 的定義（不是載重因子）。補上 F_u 的完整五段式（含 T→0 的等加速度區 F_u = 1.0）與 T_0^D = S_D1/S_DS 的分界說明——原文把分界寫成「0.3～0.5 秒」是錯的。
  - **SD-2020-4**（數值最嚴重）：原「F_u：高韌性 RC = 4.8／中韌性 2.4／低韌性 1.2」——**這是韌性容量 R 不是 F_u**。R = 4.8 → R_a = 3.533 → F_u = 3.53（長週期）或 2.46（中週期）。連帶把 V*/V 從錯誤的 1.6 更正為 **1.18**，並釐清臨界條件為「長週期且 R > 4.0」（中週期需 R > 7，超出規範表列，故實務上不會翻轉）。補推 V_M/V 的閉合式 **1 + 0.5/R**（因 S_aM = 1.5S_aD 而 F_uM 以 R、F_u 以 R_a 計算，兩者部分抵銷）——R = 4.8 時 V_M 只比 V 大 10%，這是原文完全沒答到的另一半。另修正：下標 m 原寫「取週期範圍內最大值」→ 應為規範 2.2 的**修正值**（已補完整三段式，兩分界點連續性 0.3→0.30、0.8→0.56 已驗算）；工址地盤「分 4 類」→ **3 類**（依 V_S30）；反應譜式改用規範記號的四段式；補上 4.2（一般工址與近斷層）vs 3.5（臺北盆地）的區別。
  - **SD-2018-3**（觀念，由實算推翻原敘述）：原文稱「軟層與質量不規則使兩個假設均失效」「軟層使第一振態有效質量下降」。以 5 層剪力屋架解廣義特徵值問題（先對稱化 A = M^(-1/2)KM^(-1/2)，避免 inv(M)K 非對稱的解錯）實算後**推翻**：軟層在 1F 時 m₁* 反而由 88.0% **升到 95.5%**（上部趨近剛體平移，更像 SDOF），失效的是振型形狀（偏離直線 0.164 → 0.321）；質量不規則在 1F 才使二振態有效質量 8.7% → **16.3%**，破壞的是第一振態主導。改寫為「**兩種不規則各打破一個假設**」，並補上原文缺少的核心推導——把 φ_i1 = h_i/H 與 m_i = w_i/g 代入 f_ir = Γ_r S_a m_i φ_ir，規範的 F_i = w_ih_i/Σw_jh_j · V 會自己掉出來，證明規範式是動力式的特例。另修正：軟層定義的 0.7／0.8 兩個門檻原為對調（與**上一層**比 0.70、與**上三層平均**比 0.80）；§4.5 原有「振幅偏凸」與「幅值偏小」自相矛盾的兩句，改以剪力重分配重寫；§5「規範規定軟層須提高設計地震力乘放大係數」查無依據，改為動力分析基底剪力下限的正確敘述。
  - **SD-2012-3**（格式＋數值）：原檔**不符 CLAUDE-SOLVE 標準格式**（無標籤區塊、無 §1–§5、無 §3.5 變數層次分析），是 log 中 2026-08-22 記載的「舊式格式 10 頁」之一，已完整改寫為標準格式（此項待辦可自該清單移除 1 頁）。內容更正：層間位移角限值原寫「1/200 或 1/400」→ 規範 2.16.1 明定**不得超過 0.005**；補上 V* 的定義與中小度地震的性能定位、扭矩放大係數 A_x ≤ 3.0、偶然偏心 ±5% 的規範原文措辭，以及「扭轉不規則→平移與扭轉振態頻率接近→必須用 CQC 而非 SRSS」的完整因果鏈。

  **/struct-diagram：15 張向量圖 + 5 支產圖腳本**，存於 `raw/solutions/<題號>/figs/`（SVG 給 wiki／WeasyPrint，2× PNG 給 pptx／影片管線），腳本 `gen_<題號>.py` 與解析同層、可重跑。所有幾何與數值皆由題目條件算出（SD-2018-3 的三張圖直接解特徵值問題，SD-2012-3 圖 2 由構架勁度算出剛心、扭轉勁度與兩端位移比 1.333），無一處硬寫座標。`render.py` 驗證 15/15 通過（0 XML 錯誤、0 溢出），並逐張目視檢查過中文字型、標註重疊與數值一致性。**SD-2018-3 的三張圖即是上述推翻原敘述的證據來源**，`gen_SD-2018-3.py` 檔頭已記錄該結論。

  **下游同步：**
  - `study/problems-view/` 五頁以現行 .md 重新渲染（含 15 張 SVG 的相對路徑改寫）。
  - **順帶修好全庫既有缺陷：99 頁 problems-view 的副標把民國年印成西元年**（「2012 年（民國 2012）」→「（民國 101）」）。
  - `wiki/problems/` 五頁重新產生（改以「涉及陷阱」單一區塊取代原本與「解題關鍵步驟」重複的內容，並新增「圖解」區塊連往 SVG）。
  - `raw/json/question_index.json`：三題標籤依修正後的 .md 檔頭同步（SD-2007-1 8→13、SD-2020-4 13→18、SD-2012-3 17→16）；另對帳 hasViz，更正 SD-2025-1、SD-2025-2 兩題（實際有 `*-viz.html` 卻標 false）。

  **未處理、僅登錄的待辦：**
  （a）**SD-2024-3 有兩處與本次相同的錯誤**：中小度地震回歸期寫 72 年（應為 30 年）；4.2 與 3.5 說成「短週期區／長週期區」的地震危害比（應為**一般工址／臺北盆地**之別）。不在本次委託範圍。
  （b）`wiki/concepts/SD-C-011.md`「設計反應譜」寫的是美規 ASCE 7 的 F_a、F_v、S_s、S_1、乘 2/3 的架構，**與我國規範的 S_S^D／S_1^D 微分區表 + 三類地盤不符**；SD-2025-1 §5.2 有引用此頁，建議一併複查 `raw/json/concepts.json` 的來源條目。
