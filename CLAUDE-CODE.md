...
**觸發語句：** `ingest SD-2015-1`（Cowork 直接執行）

**前置檢查（強制）：**
```
1. 讀取 raw/json/question_index.json
2. 找到 moduleId = "SD-2015-1" 的條目
...
6.  圖形警示（SD-U1-3 梁柱桿件題目且年份 ≥ 2016 但無 pm-viz.html）
...
2. 轉換為精簡陣列格式寫入 dashboard-data.js：
   ...
   - pdf 補充筆記：掃描 raw/solutions/SD-XXXX-N/ 下所有 *.pdf，取原始檔名（含副檔名）存入陣列；
     無 PDF 的題目寫入空陣列 []
3. 讀取 `raw/json/syllabus_taxonomy.json`，提取出 `subject.id === "SD"` 的分類樹，並自動轉換成 `window.SD_TOPICS` 與 `window.SD_UNITS` 寫入 `dashboard-data.js` 中。
4. 在 wiki/log.md 追加紀錄
...
- `study SD-U1-2`（子項層級深度複習，Cowork 直接執行）
...
- 子項層級：`study/study-SD-UN-n.html`（例：`study/study-SD-U1-2.html`）
